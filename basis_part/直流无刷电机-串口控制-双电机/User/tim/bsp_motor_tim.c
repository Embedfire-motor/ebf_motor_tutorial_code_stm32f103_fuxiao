...
static void TIM_Mode_Config(void)
{
  ...
  /* 配置触发源 */
  HAL_TIMEx_ConfigCommutEvent(&motor1_htimx_bldcm, MOTOR1_TIM_COM_TS_ITRx, TIM_COMMUTATION_SOFTWARE);
  HAL_TIMEx_ConfigCommutEvent(&motor2_htimx_bldcm, MOTOR2_TIM_COM_TS_ITRx, TIM_COMMUTATION_SOFTWARE);
...
static void hall_motor1_tim_init(void)
{
  ...
  HAL_NVIC_SetPriority(MOTOR1_HALL_TIM_IRQn, 6, 0);    // 设置中断优先级
...
static void hall_motor2_tim_init(void)
{
  ...
  HAL_NVIC_SetPriority(MOTOR2_HALL_TIM_IRQn, 6, 0);    // 设置中断优先级
